...
namespace MastermindService
{
    ...
    public class MastermindService : IService
    {
        ...
        public MastermindService()
        {
            ...
            foreach (string playerData in dataHelper.getListOfPlayers())
            {
                string[] tokens = playerData.Split();
                Playerlist.Add(new Player(tokens[0], tokens[1]));
            }
           //TO-DO get the list of players from database
        }
        //LOCAL METHODS
        ...
        {
            if (!dataHelper.isExistingUser(name))
            {
                dataHelper.RegisterNewPlayer(name, pass);
            }
        }
        ...
        public bool PlayerJoinRoom(string playerName, string roomName)
        {
            ...
            if (!roomToJoin.IsFull())
            {
                ...
                getPlayerByName(playerName).gameFinishedCallback = OperationContext.Current.GetCallbackChannel<IService1Callback>();
        ...
        public void PlayerCreateRoom(string playerName)
        {
            ...
            getPlayerByName(owner.Name).currentRoom = newRoom.OwnerName;
            owner.gameFinishedCallback = OperationContext.Current.GetCallbackChannel<IService1Callback>();
            newRoom.playersIn.Add(owner);
            //TODO CALLBACK to all other players to update the roomlist
        }

        // Here is the method to let the users in a room playing a game that the game was finished.
        public void GameOver(string roomName)
        {
            Room myRoom = getRoomByName(roomName);
            foreach (Player p in myRoom.playersIn)
            {
                p.gameFinishedCallback.gameFinished("The game is finished");
            }
        }
        ...
        public List<int> GetHiddenNumber(string RoomName)
        {
            Room r =this.getRoomByName(RoomName);
            if (r != null)
            {
                return r.currentGame.HiddenNumbers;
            }
            return new List<int>();
        }
